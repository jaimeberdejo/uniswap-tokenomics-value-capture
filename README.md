...
📄 **Informe completo:** [`analysis/report.pdf`](analysis/report.pdf) (en español, ~24 páginas)
📊 **Dashboard público (Dune):** https://dune.com/jbs19022909/uni-value-capture-after-unification
📓 **Notebook reproducible:** [`notebooks/ml-analysis.ipynb`](notebooks/ml-analysis.ipynb) — corre de principio a fin desde los CSV cacheados, **sin clave de Dune**.
...
```
analysis/
  report.pdf / .md / .html      # Informe final (ES) + fuente Markdown + HTML autocontenido
  report.css                    # Estilo de impresión (Markdown → HTML → PDF vía pandoc + navegador)
  ai-light-interpretation.md    # Capa AI-Light: interpretación
  ai-light-stress-test.md       # Capa AI-Light: stress-test adversarial
  figures/                      # Figuras del informe (PNG) generadas por el notebook
notebooks/
  ml-analysis.ipynb             # Notebook reproducible (DEL-03) — corre desde data/ sin clave
  ml_analysis.py                # Módulo companion del notebook
  _build_ipynb.py               # Reconstruye el .ipynb desde el .py
tests/
  test_ml_analysis.py           # Tests del pipeline ML (calibración, k-selección, whitelist)
...
python notebooks/ml_analysis.py

# O abrir el notebook:
jupyter notebook notebooks/ml-analysis.ipynb

# Tests:
python -m pytest tests/test_ml_analysis.py
```
